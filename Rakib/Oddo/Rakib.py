...
with open("log_results.txt", "a") as log_file:
    for _, row in data.iterrows():
        ...
        driver.find_element(By.NAME, "order_number").send_keys(row["order_number"])
        ...
        # Convert dates to strings and ensure leading zeros are present
        incoming_date = f"{int(row['incoming_date']):08d}"  # Ensure 8-digit format, leading zeros
        delivery_last_date = f"{int(row['delivery_last_date']):08d}"  # Ensure 8-digit format, leading zeros

        # Input dates into the form with added delay
        ...
        if driver.current_url != "https://www.bdcallingit.com/portal/sales/create/status":
            log_message = f"{row['order_number']} incomplete\n"
